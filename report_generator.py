...
class ReportGenerator:
    def __init__(self):
        ...
        # On Windows: Microsoft JhengHei, on Linux: Noto Sans CJK, WenQuanYi
        self.font_path = None
        potential_fonts = [
    ...
    def generate_closing_report(self, sentiment_data, stock_list, output_path="closing_report.png"):
        ...
        try:
            ...
            subtitle_font = ImageFont.truetype(self.font_path, 28) if self.font_path else ImageFont.load_default()
            body_font = ImageFont.truetype(self.font_path, 24) if self.font_path else ImageFont.load_default()
            small_font = ImageFont.truetype(self.font_path, 20) if self.font_path else ImageFont.load_default()
        except:
        ...
        # --- Stock List Title ---
        draw.text((40, 430), "監控標的盤後統計", font=subtitle_font, fill=self.text_color)
        
        # Table Header
        header_y = 480
        draw.text((60, header_y), "股票名稱", font=body_font, fill="#AAAAAA")
        draw.text((320, header_y), "收盤", font=body_font, fill="#AAAAAA")
        draw.text((450, header_y), "漲跌", font=body_font, fill="#AAAAAA")
        draw.text((600, header_y), "MA20 狀態", font=body_font, fill="#AAAAAA")
        draw.text((820, header_y), "成交量", font=body_font, fill="#AAAAAA")
        
        draw.line([60, header_y+40, 940, header_y+40], fill="#333333", width=2)
        
        # Render Rows
        curr_y = header_y + 60
        for stock in stock_list:
            if curr_y > height - 100: break # Avoid overflow
            ...
            draw.text((60, curr_y+25), f"{stock['symbol']}", font=small_font, fill="#888888")
            
            # Price
            draw.text((320, curr_y), f"{stock['close']}", font=body_font, fill=self.text_color)
            
            # Change %
            change_str = f"{stock['change_pct']:+.2f}%" if stock['change_pct'] is not None else "---"
            draw.text((450, curr_y), change_str, font=body_font, fill=color)
            ...
            draw.text((600, curr_y), ma_status, font=body_font, fill=ma_color)
            
            # Volume
            volume_str = f"{stock.get('volume', 0):,}"
            draw.text((820, curr_y), volume_str, font=body_font, fill="#AAAAAA")
            
            curr_y += 70
            draw.line([60, curr_y-10, 940, curr_y-10], fill="#222222", width=1)
            
        # Footer
        draw.text((400, height - 60), "Antigravity Stock Monitor v2.0", font=small_font, fill="#555555")
